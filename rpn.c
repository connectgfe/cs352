...
* Purpose: use a stack to perform rpn calculations
*/
...
/*
     if(strcmp(instr1,zero)==0 && strcmp(instr2,zero)==0){
      zero=strdup(z); 
      return zero;
     }
*/
...
//   printf("%d %d\n",n,l1);
  ...
  memset(tstr1,0,(lenF+1)*sizeof(char));
    char tstr2[lenF+1];
   strcpy(tstr1,str1);
   strcpy(tstr2,str2); 
   // this sub
...
//    free(num);
    ...
    int p=0;  
    size_t nlen2=strlen(num);
    for(p=nlen2-1;p>-1;p--){
    if(num[p]=='0'){ num[p]='\0';}else{break;}}



//   if(num[nlen2-1]=='0'){ num[nlen2-1]='\0';}   
