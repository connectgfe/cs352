...
void addvert(vert *verhead, char *name, graph *grph, int targ){
  ...
  while(cur->next!=NULL){
   ...
   cur=cur->next;
     ...
     cur->vname=name;
     cur->targ=targ;
     cur->targof=(int*)cs352_calloc(30,sizeof(int)); 
   ...
   printf("%s %d %d\n",cur->vname, cur->vind, cur->targ);
    cmnd *ccur=cur->cmd;
      while(ccur->nextc!=NULL){
          ...
          printf("edge of: %d %d %d\n",*(cur->targof),*(cur->targof+1),*(cur->targof+2));
 ...
 vert *ncur=verhead;
   ...
   //        printf("edgeof:%d\n",*ncur->edgeof);
          ...
          if(*(ncur->targof+j)!=0){
           ...
           if(indir(ncur->targof,verhead)==1){
              myfree(verhead,grph);
              fclose(fp); 
              free(testln); 
              free(vtx); 
...
// printf("retval:%d\n",retval);
             if(retval!=0){
              ...
              free(testln); 
              free(vtx); 
              fprintf(stderr,"Error: gcc failure.\n");
             exit(1); 
             }
...
int freeval=0;
   ...
   while(*(edgelist+j)!=0){
     ...
     while(cur->next!=NULL){
        ...
        if(*(edgelist+j)==cur->vind){
         val1 = scandir(".", &dirlist,NULL,alphasort);
         freeval=val1;
           while(val1--){
//printf("%s %s\n",dirlist[val1]->d_name,cur->vname);
